-- Check recent goals and their task counts
SELECT 
  g.id,
  g.title,
  g.status,
...
SELECT 
  ...
  CEIL(g.plan_duration_days / 7.0) as total_weeks,
  COALESCE(ARRAY_LENGTH(g.preferred_days, 1), 7) as days_per_week,
  LEAST(
