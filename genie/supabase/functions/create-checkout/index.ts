...
serve(async (req) => {
  ...
  try {
    ...
    if (type === 'tokens') {
      ...
      console.log(`💳 User ${user.id} subscription status:`, isSubscribed);
      
      const pricePerToken = 0.05; // $0.05 per token
      let totalPrice = amount * pricePerToken;
      
      console.log(`💰 Base price for ${amount} tokens: $${totalPrice}`);
      
      // Apply 15% discount for subscribers
      if (isSubscribed) {
        const originalPrice = totalPrice;
        totalPrice = totalPrice * 0.85; // 15% discount
        console.log(`🎉 Subscriber discount applied! ${originalPrice} → ${totalPrice} (15% off)`);
      }
      
      totalPrice = Math.round(totalPrice * 100); // Convert to cents
      console.log(`💳 Final price in cents: ${totalPrice}`);
